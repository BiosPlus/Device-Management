...
# installing a new copy of the template.
  ...
  if [[ ! -f "$touch_id_auth_file" ]]; then
     ...
     runAsUser /usr/local/bin/dialog --notification --title "Sudo TouchID" --message "Sudo auth for TouchID has been enabled. Please close any existing terminal windows and open a new one to test."
   ...
   # been.
     ...
     runAsUser /usr/local/bin/dialog --notification --title "Sudo TouchID" --message "An error has arisen in modifying the /etc/pam.d/sudo_local file. Please inform IT."
